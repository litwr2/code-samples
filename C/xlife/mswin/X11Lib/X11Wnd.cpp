...
bool X11Wnd::createWindow(int x, int y, int w, int h)
{
	...
	m_hWnd = ::CreateWindowEx(0,"X11Wnd","",
		WS_BORDER|WS_CHILD|WS_CLIPSIBLINGS|WS_CLIPCHILDREN,x,y,w + 2,h + 2,
...
{
	::SetWindowPos(m_hWnd,0,x - 1,y - 1,0,0,
...
{
        if (m_parentWnd == NULL)//for main window
	{
		RECT r;
		::GetWindowRect(m_hWnd,&r);
		int width = r.right - r.left, height = r.bottom - r.top;
		::GetClientRect(m_hWnd,&r);
		width -= r.right - r.left + sizeDelta;//size of 2 borders in pixels
		height -= r.bottom - r.top + sizeDelta;//size of title and 1 border in pixels
		::SetWindowPos(m_hWnd,0,0,0,w + width,h + height,
			SWP_NOACTIVATE|SWP_NOCOPYBITS|SWP_NOOWNERZORDER|SWP_NOMOVE|SWP_NOZORDER);		
	}
	else
	{
		::SetWindowPos(m_hWnd,0,0,0,w + 2,h + 2,
			SWP_NOACTIVATE|SWP_NOCOPYBITS|SWP_NOOWNERZORDER|SWP_NOMOVE|SWP_NOZORDER);
	}
}
...
void X11Wnd::drawLine(Context* context, int x1, int y1, int x2, int y2)
{
	...
	::LineTo(m_dc,x2+sgn(x2-x1),y2+sgn(y2-y1));
	::InvalidateRect(m_hWnd,NULL,FALSE);
}
...
{
	setContext(context);
	::Rectangle(m_dc,x,y,x+w+1,y+h+1);
	::InvalidateRect(m_hWnd,NULL,FALSE);
}
...
{
	setContext(context);
	if (get_m_drawMode(context) == R2_XORPEN)
		::SetTextColor(m_dc, get_m_foreColour(context) ^ ::GetPixel(m_dc,x+3,y-3));	
...
void X11Wnd::drawRects(Context* context, RECT* rects, int numRects)
{
	setContext(context);
	for (int i = 0; i < numRects; i++)
		::Rectangle(m_dc,rects[i].left,rects[i].top,rects[i].right,rects[i].bottom);
	::InvalidateRect(m_hWnd,NULL,FALSE);
}
