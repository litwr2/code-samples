...
namespace
{
	struct passwd userPassword = { (char*)"winuser",1,1,(char*)"",(char*)"",(char*)"winuser" };
}
...
extern "C" char* getcwd(char *dir, size_t size)
{
        int len;
   	if (len = ::GetCurrentDirectory(size, (LPTSTR)dir))
   	{
	   	char *p;
   		while (p = strchr(dir, '\\')) *p = '/';
		return dir;
        }
	return NULL;
}
...
#define NUMBER_COLOURS 9
#define NUMBER_FONTS 9
#define NUMBER_KEYS 36
namespace
{
	Display display;
	Screen screen;
	...
	NamedColour namedColours[NUMBER_COLOURS] = {
		(char*)"black", RGB(0x00,0x00,0x00),0x0000,0x0000,0x0000,
		(char*)"blue",  RGB(0x00,0x00,0xFF),0x0000,0x0000,0xFFFF,
		(char*)"brown", RGB(0xA0,0x70,0x20),0xA000,0x7000,0x2000,
		(char*)"green", RGB(0x00,0xFF,0x00),0x0000,0xFFFF,0x0000,
		(char*)"purple",RGB(0xFF,0x00,0xFF),0xFFFF,0x0000,0xFFFF,
		(char*)"red",   RGB(0xFF,0x00,0x00),0xFFFF,0x0000,0x0000,
		(char*)"white", RGB(0xFF,0xFF,0xFF),0xFFFF,0xFFFF,0xFFFF,
		(char*)"yellow",RGB(0xFF,0xFF,0x00),0xFFFF,0xFFFF,0x0000,
                (char*)"orange",RGB(0xFF,0xA5,0x00),0xFFFF,0xA500,0x0000,
	};
	...
	NamedFont namedFonts[NUMBER_FONTS] = {
                "5x9",fixedFontNames,5,9,FW_NORMAL,
                "6x8",fixedFontNames,6,8,FW_NORMAL,
                "6x9",fixedFontNames,6,9,FW_NORMAL,
                "6x10",fixedFontNames,6,10,FW_NORMAL,
                "7x11",fixedFontNames,7,11,FW_NORMAL,
	...
	KeyTranslation keys[NUMBER_KEYS] = {
		...
		VK_NUMPAD5,XK_KP_5,XK_KP_Begin,
		...
		VK_END,XK_End,XK_End,
                VK_HOME,XK_Home,XK_Home,
                VK_PRIOR,XK_Page_Up,XK_Page_Up,
                VK_NEXT,XK_Page_Down,XK_Page_Down,
                VK_INSERT,XK_KP_Insert, XK_KP_Insert,
                VK_CLEAR,XK_KP_Begin,XK_KP_Begin,
	};
}
...
extern "C" Status XAllocNamedColor(Display* display, Colormap colormap, _Xconst char*	color_name, XColor* screen_def_return, XColor* exact_def_return)
{
	...
	int i = 0;
        ...
        {
          int r, g, b;
          r = strtol(color_name + 1, 0, 16);
          b = r & 255;
          r >>= 8;
          g = r & 255;
          r >>= 8;
          screen_def_return->pixel = RGB(r,g,b);
	  screen_def_return->red = r << 8;
	  screen_def_return->green = g << 8;
	  screen_def_return->blue = b << 8;
	  found = true;
          goto FIN;
        }
...
extern "C" Bool XCheckMaskEvent(Display* display, long event_mask, XEvent* event_return)
{
	...
	if (X11App::getApp().getFromMsgQueue(msgs,msg))
	{
		...
		if( ::GetAsyncKeyState(VK_LSHIFT) & 0x8000)//bad shift detect
...
{
	((X11Wnd::Context*)gc)->setForegroundColour(foreground);
	return 0;	
}
...
extern "C" int XSetWindowBackground(Display*, Window wnd, unsigned long background)
{
...
{
	((X11Wnd*)d)->drawString((X11Wnd::Context*)gc,x,y+4,string,length);
	return 0;
}
...
extern "C" int XSelectInput(Display* display, Window w, long event_mask)
{
	...
	if (event_mask & ButtonPressMask)
	{
		...
		((X11Wnd*)w)->addMessage(WM_MBUTTONDOWN);
                ((X11Wnd*)w)->addMessage(WM_MOUSEWHEEL);
	}
	if (event_mask & ButtonReleaseMask)
	{
        ...
        if(event_mask & ExposureMask)
			((X11Wnd*)w)->addMessage(WM_NCCALCSIZE);
	return 0;
}
...
extern "C" void XSetWMName(Display* display, Window w, XTextProperty* window_name)
{
		((X11Wnd*)w)->setWindowTitle((char*)(window_name->value));
}
