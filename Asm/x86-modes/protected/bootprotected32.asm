;turns CPU to the protected mode with the default use of 32-bit code
...
;reboots after ESC key pressed
	...
	cli				; disable the interrupts
        ...
        mov bx, 16                    ;selector to the 2nd descriptor in GDT
        mov ds, bx
        mov es, bx
        mov ss, bx
        ;mov fs, bx
        ;mov gs, bx
        jmp 8:pmode                 ;use selector to the 1st descriptor of GDT

        USE32
pmode:  mov esp,0x7c00
        lidt    [IDTR]
        sti
...
;Code Selector Descriptor
         ...
         db 0xC0 or 0xf ;G=1, D=1 (USE32), limit 16..19
         db 0x00        ;base 24..31
...
repeat 8                                ;8 exceptions
      dw exception and 0xFFFF           ;low 16 bits of offset
      dw 8                              ;the selector
      dw 0x8f00                         ;P=1, PL=00, SYS=0, type = 0xf = 32 bit trap gate
      dw exception shr 16               ;high 16 bits of offset
end repeat

      dw clock and 0FFFFh               ;timer at INT 8
      dw 8
      dw 0x8e00,clock shr 16            ;P=1, PL=00, SYS=0, type = 0xe = 32 bit interrupt gate

      dw keyboard and 0xFFFF            ;keyboard at INT 9
      dw 8
      dw 0x8e00,0

repeat 1
      dw interrupt and 0FFFFh,8         ; handler for all other interrupts
      dw 0x8e00,interrupt shr 16
end repeat
EndOf_IDT:
...
beep:   in	al,61h			; turn on the speaker
	...
	out	61h,al
        ...
        mov     ecx,10                  ;100/182=100/91 approx = 0.55 sec
